...
void createFile (unsigned char *fileName)
{
    ...
    struct dir_Structure *dir;
    unsigned long cluster, nextFree, prevCluster, firstSector, sector, lastRootCluster;
    unsigned long size;
    ...
    while(1)
    {
	lastRootCluster = cluster;
	cluster = (getSetNextCluster (lastRootCluster, GET, 0));
    ...
    nextFree = searchNextFreeCluster(cluster);
    if(nextFree == 0)
    {
	TX_NEWLINE;
    ...
    firstClusterHigh = (unsigned int) ((nextFree & 0xffff0000) >> 16 );
    firstClusterLow = (unsigned int) ( nextFree & 0x0000ffff);
...
//    if(error) {transmitString_F(PSTR("Write failed..")); return;}
    ...
    getSetFreeCluster (NEXT_FREE, SET, nextFree); //update FSinfo next free cluster entry

    if(nextFree > lastRootCluster)
    {
	getSetNextCluster (lastRootCluster, SET, nextFree+1);   //mark new end of the root directory
	getSetNextCluster (nextFree+1, SET, EOF);   //last cluster of root, marked EOF
    }
